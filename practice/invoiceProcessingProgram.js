...
function createInvoice(services = {}) {
  ...
  return {
    ...
    paymentTotal: function() {
      return this.payments.reduce((sum, payment) => sum + payment.totalP(), 0); //ref. line 98 
    },

    amountDue: function() {
      return this.totalInvoice() - this.paymentTotal();
    },
  };
}
...
function invoiceTotal(invoices) {
  ...
  for (let index = 0; index < invoices.length; index += 1) {
    total += invoices[index].totalInvoice();
  }
...
function paymentTotal(payments) {
  return payments.reduce((sum, payment)  => sum + payment.totalP(), 0);
}
