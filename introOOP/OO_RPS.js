...
const RSPGame = {
  ...
  displayHistory() {
    ...
    for (let round = 0; round < this.history.length; round++) {
      roundSumUp['Round ' + Number(round + 1)] = {human: this.history[round][0], computer: this.history[round][1] ,winner: this.history[round][2]};
    }
  ...
  historyAnalysis(player, move) {
    let roundsLength = this.history.filter(round => round[1] === move).length;
    let allMovesByPlayer = this.history.filter(round => round[1] === move && round[2] !== player && round[2] !== 'tie').length;
    return roundsLength > 0 ? (allMovesByPlayer * 100) / roundsLength : 0;
  },

  displayRoundSituation() {
    console.log('Current points: ');
    console.log(`Player ${this.human.pointsCounter} - Computer ${this.computer.pointsCounter}`);
    console.log(' ');
  },

  displayGameWinner() {
    console.log('The game has ended.');
    console.log(`${this.human.pointsCounter > this.computer.pointsCounter ? 'Player' : 'Computer'} is the winner of the game!`);
    console.log(' \n ----------------- \n');
  },
